 ...
 * NOTE methods: .prepare(), withData(map) & withData(string) are build to allow caller to chain in different variations, examples:
 ...
 *
 * Example 4://
 *  HttpRequest req=new HttpRequest("https://192.168.220.82/sguap/sgemm/rest/monitorex/app/rule?os=Android&packagename=com.emm.sandboxsdk", true);
    req.prepare(HttpRequest.Method.GET).setUserAgent("msmsdk").sendAndReadString();
 */
public class HttpRequest {
    ...
    public HttpRequest(URL url, boolean ignore)throws IOException{
        if(ignore) {
            ignoreCert();
        }
        this.url=url;
    ...
    public HttpRequest(String url, boolean ignore)throws IOException{ this(new URL(url), ignore); }
    ...
    private HttpRequest ignoreCert(){
        try{
    ...
    public Certificate [] getServiceCertificates(){
        Certificate [] cf = null;
        try {
            cf = ((HttpsURLConnection)con).getServerCertificates();
        } catch (Exception e) {
            e.printStackTrace();
        }
