...
//                            }catch (Exception e){
//                                //socket已经断开
//                                Log.i(TAG,tcb.ipAndPort+" has been closed. tcp status="+tcb.status);
//                                synchronized (tcb){
...
//            TCB.closeTCB(tcb);
    ...
    private void processInput(SelectionKey key, Iterator<SelectionKey> keyIterator)
    {
        ...
        try {
            ...
            {
                if (tcb.window < 1460) {
                    Log.i(TAG,"processInput, tcb.window <= 1460 , window : " + tcb.window);
                    return;
                }

//                MsmLog.print("myvpn --- processInput, tcb.window : " + tcb.window);
//                receiveBuffer = ByteBufferPool.acquire(tcb.window);
                receiveBuffer = ByteBufferPool.acquire(1460);
                ...
                try {
                    ...
                    //change by wzl
                    Log.i(TAG,"Network read error: " + tcb.ipAndPort, e);
...
//                    Log.i(TAG,"send " + re.ipHeader.destinationAddress.getHostAddress()+":"+re.tcpHeader.destinationPort + " RST");
//                    receiveBuffer.position(referencePacket.IP_TRAN_SIZE);
...
//                    countRead += readBytes;
//                    MsmLog.print("myvpn --- result read, processInput 长度：" +  readBytes + ", countRead : " + countRead);
//                }
                }

                //通过inputChannel.read从服务器读取了数据, 扔到接口里给客户端
//                MsmLog.print("myvpn --- result read, limit :" +  ", sequenceNumber :"+ tcb.mySequenceNum);
                outputQueue.offer(receiveBuffer);
            }
        } catch (Exception e) {
            e.printStackTrace();
 ...
 */
    private void sendFIN(TCB tcb, ByteBuffer buffer){
        if(tcb.status == TCB.TCBStatus.LAST_ACK){
            return;
        }
        tcb.status = TCB.TCBStatus.LAST_ACK;
